...
def create_causal_attention_mask(seq_len, window_size):
    mask = torch.full([seq_len, seq_len], float("-inf"))
    
    for i in range(seq_len):
        start_index = max(0, i - window_size + 1)
        mask[i, start_index:i+1] = 0
...
# import os
#
# if not os.path.exists("custom_masks.pkl"):
#     custom_masks = [None] + [create_causal_attention_mask(i, 35*5) for i in range(1, 2000)]
#     pickle.dump(custom_masks, open("custom_masks.pkl", "wb"))
# else:
#     custom_masks = pickle.load(open("custom_masks.pkl", "rb"))

custom_masks = [None] * 4000
...
class HubertSdpaAttention(HubertAttention):
    ...
    def forward(
        ...
        mask = deepcopy(custom_masks[seq_len]).to("cuda")
        masks = mask.repeat(btz, 1).reshape(attention_mask.shape)
